...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    const res = await axios.post('https://snipai.onrender.com/api/auth/login', { email, password });
    setToken(res.data.token);
    setUser(res.data.user);
  };

  const register = async (username, email, password) => {
    await axios.post('https://snipai.onrender.com/api/auth/register', { username, email, password });
  };
